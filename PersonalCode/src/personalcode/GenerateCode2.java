...
public class GenerateCode2 {
    ...
    public GenerateCode2(String gender, String date) {
        if (checkGender(gender) && isDateValid(date)) {
            this.gender = gender;
            this.date = date;
            this.dataOk = true;
            this.strDay = date.split("\\-")[2];
            this.strMonth = date.split("\\-")[1];
            this.strYearShort = date.split("\\-")[0].substring(0);
        } else {
            this.dataOk = false;
        }
    }
...
//        String year = date.split("\\-")[0]; // Aeg on 31.12.2000 => 2000
